...
function getScore(xy: { x: number, y: number } | undefined, visited9: Set<string>, history: { x: number, y: number }[]) {
    if (!xy) return 0
    const { x, y } = xy
    ...
    isInMap({ x: x + 1, y })].reduce<number>((score_, potentialNeighboor) => {
        if (potentialNeighboor && topologyMap[potentialNeighboor.y][potentialNeighboor.x] === currentHeight + 1) return score_ + getScore(potentialNeighboor, visited9, history ? [...history, { x: potentialNeighboor.x, y: potentialNeighboor.y }] : [{ x: potentialNeighboor.x, y: potentialNeighboor.y }]);
        return score_
    }, 0)
...
const scores = trailHeads.reduce((scoresAcc, trailHead) => {
    return scoresAcc + getScore(trailHead, new Set(), [trailHead]);
}, 0)
